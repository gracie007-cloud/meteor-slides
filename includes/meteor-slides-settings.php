<?php
	...
	function meteorslides_section_text() {
		...
		echo '<p>' . esc_html__( 'Set up your slideshow using the options below.', 'meteor-slides' ) . '</p>';
	...
	function meteorslides_slideshow_quantity() {
		...
		echo '<input id="slideshow_quantity" name="meteorslides_options[slideshow_quantity]" size="20" type="text" value="' . esc_attr( $meteor_options['slideshow_quantity'] ) . '" /> ' . esc_html( $meteor_slides );
	...
	function meteorslides_slide_height() {
		...
		echo '<input id="slide_height" name="meteorslides_options[slide_height]" size="20" type="text" value="' . esc_attr( $meteor_options['slide_height'] ) . '" /> ' . esc_html( $meteor_px );
	...
	function meteorslides_slide_width() {
		...
		echo '<input id="slide_width" name="meteorslides_options[slide_width]" size="20" type="text" value="' . esc_attr( $meteor_options['slide_width'] ) . '" /> ' . esc_html( $meteor_px );
	...
	function meteorslides_transition_style() {
		...
		foreach ( $meteor_item as $meteor_key => $meteor_val ) {

			$meteor_selected = ( $meteor_options['transition_style']==$meteor_key ) ? ' selected="selected"' : '';
		
			echo '<option value="' . esc_attr( $meteor_key ) . '"' . esc_attr( $meteor_selected ) . '>' . esc_html( $meteor_val ) . '</option>';
	...
	function meteorslides_transition_speed() {
		...
		$meteor_milliseconds = __( 'milliseconds', 'meteor-slides' );
		$meteor_options = get_option( 'meteorslides_options' );

		echo '<input id="transition_speed" name="meteorslides_options[transition_speed]" size="20" type="text" value="' . esc_attr( $meteor_options['transition_speed'] ) . '" /> ' . esc_html( $meteor_milliseconds );
	...
	function meteorslides_slide_duration() {
		...
		echo '<input id="slide_duration" name="meteorslides_options[slide_duration]" size="20" type="text" value="' . esc_attr( $meteor_options['slide_duration'] ) . '" /> ' . esc_html( $meteor_milliseconds );
...
<div class="wrap">
	...
	<h2><?php esc_html_e( 'Meteor Slides Settings', 'meteor-slides' ); ?></h2>
	...
	<h3><?php esc_html_e( 'Add Slideshow', 'meteor-slides' ); ?></h3>
	
	<p><?php 
		// translators: %1$s: PHP template tag code, %2$s: shortcode
		echo wp_kses_post( sprintf( __( 'Use %1$s to add this slideshow to your theme, use %2$s to add it to your Post or Page content, or use the Meteor Slides Widget.', 'meteor-slides'), "<code><&#63;php if ( function_exists( 'meteor_slideshow' ) ) { meteor_slideshow(); } &#63;></code>", "<code>[meteor_slideshow]</code>" ) ); 
	?></p>
	
	<p><?php 
		// translators: %1$s: opening link tag, %2$s: closing link tag
		echo wp_kses_post( sprintf( __( 'Visit the %1$sMeteor Slides homepage%2$s for documentation, tutorials, and videos.', 'meteor-slides' ), "<a href='http://www.jleuze.com/plugins/meteor-slides/'>", "</a>" ) ); 
	?></p>
	
	<p><em><?php 
		// translators: %1$s: opening link tag, %2$s: closing link tag
		echo wp_kses_post( sprintf( __( 'Please %1$spost any questions or problems%2$s in the WordPress.org support forums.', 'meteor-slides' ), "<a href='http://wordpress.org/tags/meteor-slides?forum_id=10#postform'>", "</a>" ) ); 
	?></em></p>
