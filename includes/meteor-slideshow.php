...
*/
	...
	$meteor_nav      = isset( $meteor_options['slideshow_navigation'] ) ? $meteor_options['slideshow_navigation'] : 'navnone';
	$meteor_count    = 1;
	$slideshow_quantity = isset( $meteor_options['slideshow_quantity'] ) ? intval( $meteor_options['slideshow_quantity'] ) : 5;
	
	$meteor_loop     = new WP_Query( array(
	
		'post_type'      => 'slide',
		'slideshow'      => $slideshow,
		'posts_per_page' => $slideshow_quantity
	...
	<div id="meteor-slideshow<?php echo esc_attr( $slideshow ); ?>" class="meteor-slides <?php
	
		// Adds classes to slideshow
	
		echo esc_attr( $slideshow . ' ' . $meteor_nav );
		
		if ( $meteor_loop->post_count == 1 ) {
		
			echo esc_attr( ' single-slide' );
		...
		if ( !empty( $slideshow ) ) {
			...
			echo esc_js( "next: '#meteor-next" . esc_attr( $slideshow ) . "', prev: '#meteor-prev" . esc_attr( $slideshow ) . "', pager: '#meteor-buttons" . esc_attr( $slideshow ) . "'" );
	...
	if ( $meteor_loop->post_count > 1 ) : ?>
		...
		if ( $meteor_nav == "navboth" ) : ?>
			...
			<ul class="meteor-nav">
				...
				<li id="meteor-prev<?php echo esc_attr( $slideshow ); ?>" class="prev"><a href="#prev"><?php esc_html_e( 'Previous', 'meteor-slides' ) ?></a></li>
			
				<li id="meteor-next<?php echo esc_attr( $slideshow ); ?>" class="next"><a href="#next"><?php esc_html_e( 'Next', 'meteor-slides' ) ?></a></li>
			
			</ul><!-- .meteor-nav -->
		
			<div id="meteor-buttons<?php echo esc_attr( $slideshow ); ?>" class="meteor-buttons"></div>
	...
	<?php endif; ?>
		...
		while ( $meteor_loop->have_posts() ) : $meteor_loop->the_post(); ?>
			...
			if ( $meteor_count == 1 && $meteor_loop->post_count > 1 ) {
				...
				echo '<img style="visibility: hidden;" class="meteor-shim" src="' . esc_url( $meteor_shim[0] ) . '" alt="" />';
				
			} ?>

			<div class="mslide mslide-<?php echo esc_attr( $meteor_count ); ?>">
				
				<?php // Adds slide image with Slide URL link
					
				if ( get_post_meta( $post->ID, "slide_url_value", $single = true ) != "" ): ?>
						
					<a href="<?php echo esc_url( get_post_meta( $post->ID, "slide_url_value", $single = true ) ); ?>" title="<?php the_title_attribute(); ?>"><?php the_post_thumbnail( 'featured-slide', array( 'title' => get_the_title() ) ); ?></a>
