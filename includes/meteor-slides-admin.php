<?php
	...
	function meteorslides_updated_messages( $meteor_messages ) {
		...
		$meteor_messages['slide'] = array( 
			...
			0  => '',
			// translators: %s: URL to view the slide
			...
			// Note: $_GET['revision'] is used here for WordPress core post update messages display only, no form processing
			// translators: %s: revision date and time
			5  => isset($_GET['revision']) ? sprintf( __( 'Slide restored to revision from %s', 'meteor-slides' ), wp_post_revision_title( (int) sanitize_text_field( wp_unslash( $_GET['revision'] ) ), false ) ) : false,
			// translators: %s: URL to view the slide
			...
			// translators: %s: URL to preview the slide
			...
			// translators: %1$s: scheduled date and time, %2$s: URL to preview the slide
	...
	function meteorslides_image_box() {
		...
		// translators: This creates a title like "Slide Image (800x600)" with width and height dimensions
	...
	function meteorslides_new_meta_box() {
		...
		foreach ( $meteorslides_new_meta_box as $meteorslides_meta_box ) {
			...
			echo '<input type="hidden" name="' . esc_attr( $meteorslides_meta_box['name'] ) . '_noncename" id="' . esc_attr( $meteorslides_meta_box['name'] ) . '_noncename" value="' . esc_attr( wp_create_nonce( plugin_basename(__FILE__) ) ) . '" />';

			echo "<input type='text' name='" . esc_attr( $meteorslides_meta_box['name'] ) . "_value' value='" . esc_attr( $meteorslides_meta_box_value ) . "' size='55' /><br />";

			echo '<p>' . esc_html__('Add the URL this slide should link to.','meteor-slides') . '</p>';
	...
	function meteorslides_save_postdata( $post_id ) {
		...
		foreach( $meteorslides_new_meta_box as $meteorslides_meta_box ) {
			...
			if ( !isset( $_POST[$meteorslides_meta_box['name'].'_noncename']  ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[$meteorslides_meta_box['name'].'_noncename'] ) ), plugin_basename(__FILE__) ) ) {
			...
			$meteor_data = isset( $_POST[$meteorslides_meta_box['name'].'_value'] ) ? sanitize_text_field( wp_unslash( $_POST[$meteorslides_meta_box['name'].'_value'] ) ) : '';
	...
	function meteorslides_custom_columns( $meteor_column ) {
		...
		switch ( $meteor_column ) {
			...
			case 'slide' :
				...
				the_post_thumbnail('featured-slide-thumb');
			...
			case 'slide-link' :
				...
				if ( get_post_meta($post->ID, "slide_url_value", $single = true) != "" ) {
					...
					$slide_url = get_post_meta($post->ID, "slide_url_value", $single = true);
					echo "<a href='" . esc_url( $slide_url ) . "'>" . esc_html( $slide_url ) . "</a>";
	...
	function meteorslides_add_contextual_help() {
		...
		if ('slide' == $meteor_contextual_screen->id ) {
			...
			'<p><strong>' . esc_html__( 'Title', 'meteor-slides' ) . '</strong> - ' . esc_html__( 'Name the slide so it can be easily found later.', 'meteor-slides' ) . '</p>' .
			'<p><strong>' . esc_html__( 'Slide Image', 'meteor-slides' ) . '</strong> - ' . esc_html__( 'To add an image to a slide, click the "Set featured image" link. Upload an image, or browse the media library for one, click the "Use as featured image" link to add the image and then close the media uploader. The Slide Image metabox should now have a thumbnail image.', 'meteor-slides' ) . '</p>' .
			// translators: %s: example URL in emphasis tags
			'<p><strong>' . esc_html__( 'Slide Link', 'meteor-slides' ) . '</strong> - ' . sprintf( esc_html__( 'Add the full URL to the Slide Link metabox, such as %s (Optional)', 'meteor-slides' ), '<em>http://jleuze.com/</em>' ) . '</p>' .
			// translators: %1$s: opening link tag, %2$s: closing link tag
			'<p><strong>' . esc_html__( 'Slideshows', 'meteor-slides' ) . '</strong> - ' . sprintf( esc_html__( 'A slide can be added %1$sto a slideshow%2$s by selecting the slideshow from the Slideshows metabox.', 'meteor-slides' ), '<a href="http://jleuze.com/plugins/meteor-slides/multiple-slideshows/">', '</a>' ) . '</p>' .
			...
			'<p><a href="http://jleuze.com/plugins/meteor-slides/using-meteor-slides/" target="_blank">' . esc_html__( 'Documentation on Creating Slides', 'meteor-slides' ) . '</a></p>' .
			'<p><a href="http://wordpress.org/tags/meteor-slides" target="_blank">' . esc_html__( 'Plugin Support Forum', 'meteor-slides' ) . '</a></p>' .
			'<p><a class="button" href="http://jleuze.com/go/paypal/" target="_blank">' . esc_html__( 'Donate', 'meteor-slides' ) . '</a></p>';
			...
			'<p><code>' . esc_html__( '[meteor_slideshow slideshow="slug"]', 'meteor-slides' ) . '</code></p>';
			...
			'<p><em>' . esc_html__( 'Before adding any slides, enter the slide height and width in the settings so the slides are the correct dimensions.', 'meteor-slides' ) . '</em></p>' .
			...
			'<p><code>' . esc_html__( '[meteor_slideshow metadata="random: 1"]', 'meteor-slides' ) . '</code></p>';
	...
	function meteorslides_options_validate( $meteor_input ) {
		...
		if ( !preg_match( '/^[0-9]+$/i', $meteor_options['transition_speed'] ) || $meteor_options['transition_speed'] > 99999 ) {
		...
		if ( !preg_match( '/^[0-9]+$/i', $meteor_options['slide_duration'] ) || $meteor_options['slide_duration'] > 99999 ) {
	...
	function meteorslides_admin_css() {
		...
		// Note: $_GET['post_type'] is used here for admin CSS loading only, following standard WordPress admin patterns
		if ( ( isset( $_GET['post_type'] ) && 'slide' == sanitize_text_field( wp_unslash( $_GET['post_type'] ) ) ) || ( isset( $post_type ) && $post_type == 'slide' ) ) {
